...
5. **Create a FOMOD only if the install has something to ask.** The default for a new release is
   ...
   **If the release ships an installer image**, follow the confirmed-working recipe in CLAUDE.md's
   ...
   baseline (not progressive) JPEG. There is no worked example left in this repo — copy
   `build/fomod-example/ModuleConfig.xml` from the `claudemoddev` template workspace verbatim and
   edit the deltas. A wrong image setup still builds and still passes `-CheckFomod`;
